...
t_node* add_new_back(t_list *list, char *answer){ /*gibt adresse des letzten nodes zur�ck*/
    ...
    cur->next=new_node;/*einh�ngen am ende*/
    /*printf("[add_new_back]: t5\n"); TEST TEST TEST*/
    return new_node;
}
...
t_list create_list(char *fn, t_list list) { /*initialisiere fn mit NULL f�r blank list*/
    FILE *fp;
    int test = 0;
...
t_node * find_elem(int n, t_list *list){ /*gibt adresse der n-ten node in list zur�ck*/
    t_node* cur=NULL;
    t_node* buf=NULL;
    int cnt=0;
...
t_node* remove_node(t_list *list, t_node* node){ /*h�ngt "node" aus "list" aus, gibt Adresse der ausgeh�ngten node zur�ck*/
    t_node* buf;
    t_node* pre;

    if(list->first_element==node){ /*sonderfall: das erste Element soll ausgeh�ngt werden */
        list->first_element=node->next;
        list->elem_cnt--;
        return node;
    }else{                   /*normalfall: zweites oder h�heres element wird ausgeh�ngt*/
...
void elem_pileChange(int n, t_list* list1, t_list *list2){ /*n-te element aus list 1 aus- und in list 2 einh�ngen*/
    t_node* buf;
    t_node* pre;
    ...
    add_new_back(list2, buf->str); /*einh�ngen in list2*/
...
void change_pile(int n, t_list*list1, t_list *list2){ /*h�nge aus list1 aus und in list2 ein*/
    t_list buf;
...
void delete_list(t_list* list){ /*ohne R�ckgabewert?*/
...
char** gimmeCards(int needed_cards, t_list* draw, t_list* discard){
    ...
    for(i=0;i<needed_cards;i++){
        ...
        stringlist[i]=malloc((strlen(node->str)+1)*sizeof(char)); /*strlen plus platz f�r null terminator*/
